...
using Test.DemoApp.Common.Exceptions;
using Test.DemoApp.Common.Interfaces;
using Test.DemoApp.Common.Logging;
using Test.DemoApp.Data.Interfaces;

namespace Test.DemoApp.Data.Services
{
	public class Test.DemoAppApiServices : ITest.DemoAppApiServices
	{
		...
		public Test.DemoAppApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new Test.DemoAppApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw Test.DemoAppApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new Test.DemoAppApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
